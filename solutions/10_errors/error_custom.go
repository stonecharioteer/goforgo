...
func (e ValidationError) Error() string {
	...
	return fmt.Sprintf("validation error: %s (value: %v) - %s", e.Field, e.Value, e.Message)
}
...
func (e NetworkError) Error() string {
	...
	return fmt.Sprintf("network error at %s: %s operation failed for URL '%s': %v",
...
// Define sentinel errors (package-level error variables)
var (
...
func fetchUserData(url string) error {
	...
	if url == "" {
		...
		return NetworkError{
			...
			URL:       url,
			Timestamp: time.Now(),
			Err:       errors.New("empty URL provided"),
		}
	}
	if url == "timeout.com" {
...
// Helper function for string contains (since we're learning)
...
func main() {
	...
	for _, age := range testAges {
		...
		if err != nil {
			...
			// Type assert to ValidationError for additional info
	...
	for _, url := range testURLs {
		...
		if err != nil {
			...
			if netErr, ok := err.(NetworkError); ok {
				...
				if wrappedErr := netErr.Unwrap(); wrappedErr != nil {
	...
	for _, op := range testOperations {
		...
		if err != nil {
			...
			default:
				...
				if accErr, ok := err.(AccountError); ok {
	...
	// Create different types of errors and demonstrate type checking
	errors := []error{
